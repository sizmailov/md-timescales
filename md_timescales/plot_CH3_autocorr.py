...
def plot_acorr_fit(path_to_fit_csv, path_to_csv_acorr, path_to_output_pdf):
    """
    Plot one pdf with a particular fit function (e.g. CH3-acorr-2-exp.pdf)
     - [ ] Show acorr data
     - [ ] Show fit curve
     - [ ] Show fit parameters
    ...
    exp_order = {2: "tau_CH3_2_exp", 3: "tau_CH3_3_exp", 4: "tau_CH3_4_exp"}
    ...
    for order in range(2,5):
        with PdfPages(os.path.join(path_to_output_pdf, exp_order[order] + ".pdf")) as pdf:
            ...
            for ind, file in enumerate(csv_files):
                ...
                ax.grid(True)
                # ax.set_legend(("CH3 autocorrelation", "fit , loc='upper right')
                pdf.savefig()
                plt.close()
