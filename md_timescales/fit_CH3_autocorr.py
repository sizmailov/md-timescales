...
def save_fit_auto_correlation(path_to_ref: str, path_to_csv_accor: str, output_directory: str):
    ...
    for order in range(2, 5):
        ...
        for file in csv_files:
            ...
            CH3_tau_table = pd.concat([CH3_tau_table, temp] )

        CH3_tau_table = CH3_tau_table.sort_values(by=['rId'])
        CH3_tau_table.to_csv(os.path.join(output_directory, 'tau_CH3_%d_exp.csv' % order), index=False)
